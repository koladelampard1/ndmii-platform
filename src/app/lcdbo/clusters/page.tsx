...
const investorBenefits = [
  { title: "Verified businesses", icon: BadgeCheck },
  { title: "Visible production capacity", icon: Factory },
  { title: "Market opportunities", icon: TrendingUp },
  { title: "Investable pipelines", icon: Banknote },
  { title: "Export potential", icon: Globe2 },
] as const;
...
export default async function LcdboClustersPage({
  ...
  searchParams: Promise<{ q?: string; state?: string; sector?: string; clusterType?: string; status?: string }>;
}) {
    ...
    <section className="relative isolate min-h-[620px] overflow-hidden bg-[#06172f] text-white">
      ...
      <div className="relative mx-auto flex min-h-[620px] max-w-7xl items-center px-4 py-16 sm:px-6">
        <div className="max-w-4xl">
          <p className="inline-flex items-center gap-2 rounded-full border border-[#efc85d]/35 bg-[#efc85d]/10 px-4 py-2 text-[11px] font-black uppercase tracking-[0.18em] text-[#efc85d] backdrop-blur"><Network className="h-4 w-4" />National Industrial Cluster Network</p>
          <h1 className="mt-6 text-5xl font-black leading-[0.98] tracking-[-0.045em] sm:text-7xl lg:text-[5.2rem]">774 Industrial Clusters.<br /><span className="text-[#efc85d]">One National Production Network.</span></h1>
          <p className="mt-6 max-w-3xl text-base leading-7 text-slate-200 sm:text-xl sm:leading-8">Every Local Government Area becomes a specialised production hub built around its strongest economic advantage, connecting MSMEs, investors, buyers and partners into a coordinated industrial ecosystem.</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href="#cluster-discovery" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#D4A017] px-5 text-sm font-black text-[#06172f] transition hover:-translate-y-0.5 hover:bg-[#efc85d]">Explore the Network <ArrowDown className="h-4 w-4" /></Link>
...
function FeaturedClustersSection() {
  return (
    <section className="bg-[#f3f6f9] px-4 py-16 sm:px-6 lg:py-20">
      <div className="mx-auto max-w-7xl">
        <SectionHeading eyebrow="Featured cluster opportunities" title="Specialised ecosystems built around local advantage." description="Illustrative profiles show how priority sectors can become coordinated production, investment and market-access platforms." />
        <div className="mt-9 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {featuredClusters.map((cluster) => <article key={cluster.name} className="group overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm transition duration-500 hover:-translate-y-1.5 hover:shadow-2xl"><div className="relative h-52 overflow-hidden"><Image src={cluster.image} alt={`${cluster.name} demonstration profile`} fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#041226]/80 to-transparent" /><span className="absolute left-4 top-4 rounded-full border border-white/15 bg-[#06172f]/80 px-3 py-1 text-[9px] font-black uppercase tracking-[0.14em] text-[#efc85d] backdrop-blur">Demonstration profile</span><div className="absolute inset-x-0 bottom-0 p-5"><p className="text-xs font-black uppercase tracking-[0.14em] text-emerald-300">{cluster.sector}</p><h3 className="mt-1 text-2xl font-black text-white">{cluster.name}</h3></div></div><div className="p-5"><div className="flex items-center justify-between gap-3 text-xs font-bold text-slate-500"><span className="inline-flex items-center gap-1.5"><MapPinned className="h-4 w-4 text-[#008751]" />{cluster.state} State</span><span>Est. {cluster.msmes} MSMEs</span></div><p className="mt-4 text-sm leading-6 text-slate-600">{cluster.opportunity}</p><div className="mt-4 rounded-xl bg-slate-50 px-3 py-2.5"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-400">Value-chain focus</p><p className="mt-1 text-xs font-black text-[#06172f]">{cluster.focus}</p></div></div></article>)}
        </div>
      </div>
    </section>
  );
...
function ClusterDiscoverySection({ clusters, query, state, sector, clusterType, status, states, sectors, clusterTypes, statuses, isFiltered }: {
  ...
  return (
    <section id="cluster-discovery" className="scroll-mt-24 bg-white px-4 py-16 sm:px-6 lg:py-20">
      <div className="mx-auto max-w-7xl">
        <SectionHeading eyebrow="Explore the live pipeline" title="Discover clusters by product, sector and place." description="Search the current LCDBO pipeline by cluster name, product, state or value-chain focus, then refine the result with programme filters." />

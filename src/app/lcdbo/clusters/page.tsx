      ...
      <div className="relative mx-auto flex min-h-[620px] max-w-7xl items-center px-4 py-16 sm:px-6">
        <div className="max-w-4xl">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <Link href={lcdboPublicHref("/opportunities")} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/25 bg-white/5 px-5 text-sm font-black text-white backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/10">View Opportunities <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </div>
    </section>
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
          ...
          <form className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-xl shadow-slate-200/70 sm:p-6">
            ...
            {hasSearch ? <Link href={lcdboPublicHref("/clusters#state-opportunities")} className="mt-4 inline-flex text-sm font-black text-[#008751]">Clear resource filters</Link> : null}
          </form>
        </div>
  ...
  return (
    <section id="cluster-discovery" className="scroll-mt-24 bg-white px-4 py-16 sm:px-6 lg:py-20">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold text-slate-500"><span className="font-black text-[#06172f]">{clusters.length}</span> cluster{clusters.length === 1 ? "" : "s"} match the current view</p>{isFiltered ? <Link href={lcdboPublicHref("/clusters#cluster-discovery")} className="text-sm font-black text-[#008751]">Clear all filters</Link> : null}</div>

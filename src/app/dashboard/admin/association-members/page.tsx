...
export default async function AdminAssociationMembersPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <section className="space-y-4">
    ...
    <form action={submitBulkAssociationMemberAction} className="space-y-3">
      {canReview && <div className="flex flex-wrap items-center gap-2 rounded-xl border bg-white p-3 text-sm"><strong>Bulk actions</strong><input name="reason" placeholder="Reason required for reject or reassignment" className="min-w-64 flex-1 rounded border px-2 py-2" /><select name="assigned_reviewer_id" className="rounded border px-2 py-2"><option value="">Reviewer</option>{workspace.reviewers.map((v) => <option key={v.id} value={v.id}>{v.label}</option>)}</select><button name="action" value="approve" className="rounded border px-3 py-2 font-bold">Approve</button><button name="action" value="reject" className="rounded border px-3 py-2 font-bold">Reject</button>{ctx.role === "admin" && <button name="action" value="assign_reviewer" className="rounded border px-3 py-2 font-bold">Assign reviewer</button>}{ctx.role === "admin" && <button name="action" value="export" className="rounded border px-3 py-2 font-bold">Export selected</button>}</div>}
      <div className="overflow-x-auto rounded-xl border bg-white"><table className="w-full text-sm"><thead className="bg-slate-100 text-left text-[11px] uppercase tracking-wide text-slate-600"><tr><th className="px-3 py-2"></th><th className="px-3 py-2">Member</th><th className="px-3 py-2">Business</th><th className="px-3 py-2">Association</th><th className="px-3 py-2">Import Source</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Readiness</th><th className="px-3 py-2">Reviewer</th><th className="px-3 py-2"></th></tr></thead><tbody>
        {workspace.rows.map((row) => <tr key={row.id} className="border-t align-top"><td className="px-3 py-3">{canReview && <input type="checkbox" name="member_id" value={row.id} />}</td><td className="px-3 py-3"><p className="font-bold">{row.fullName ?? "Unavailable"}</p><p className="text-xs text-slate-500">{row.phone ?? "No phone"} · {row.email ?? "No email"}</p></td><td className="px-3 py-3"><p>{row.businessName ?? "Unavailable"}</p><p className="text-xs text-slate-500">{row.tradeType ?? "Trade unavailable"} · {row.lga ?? "LGA unavailable"}</p></td><td className="px-3 py-3">{row.associationName ?? "Unavailable"}</td><td className="px-3 py-3">{row.importSource ?? "Manual / legacy"}</td><td className="px-3 py-3"><span className={`rounded-full border px-2 py-1 text-[11px] font-bold ${badge(row.status)}`}>{humanize(row.status)}</span>{row.duplicateSignal && <p className="mt-2 text-xs font-bold text-amber-700">Duplicate: {row.duplicateReasons.join(", ") || "flagged"}</p>}</td><td className="px-3 py-3"><span className={`rounded-full border px-2 py-1 text-[11px] font-bold ${badge(row.readiness.label)}`}>{row.readiness.label}</span></td><td className="px-3 py-3">{row.assignedReviewerName ?? "Unassigned"}</td><td className="px-3 py-3"><Link href={`/dashboard/admin/association-members/${row.id}`} className="font-black text-emerald-800">Review</Link></td></tr>)}
        {!workspace.rows.length && queueSource.available && <tr><td colSpan={9} className="px-3 py-10 text-center text-slate-500">No members match the current queue filters.</td></tr>}
        {!workspace.rows.length && !queueSource.available && <tr><td colSpan={9} className="px-3 py-10 text-center text-rose-700">Member queue is unavailable.</td></tr>}
      </tbody></table></div>
    </form>
  </section>;
}

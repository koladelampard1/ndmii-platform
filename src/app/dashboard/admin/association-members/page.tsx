...
import { AssociationMemberBulkActions } from "@/components/admin/association-member-bulk-actions";
import { ASSOCIATION_MEMBER_ACTIVATION_STATES, ASSOCIATION_MEMBER_STATUSES, associationMemberFilterSnapshot, getAdminAssociationMembersWorkspace, type AdminAssociationMemberFilters } from "@/lib/data/admin-association-members";
...
export default async function AdminAssociationMembersPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <section className="space-y-4">
    ...
    <form className="grid gap-2 rounded-xl border bg-white p-4 md:grid-cols-4 lg:grid-cols-6">
      ...
      <select name="access" defaultValue={workspace.filters.access} className="rounded border px-2 py-2 text-sm"><option value="">All access records</option><option value="not_generated">Access not generated</option><option value="ready">Access ready</option><option value="expired">PIN expired</option><option value="completed">First login completed</option><option value="password_pending">Password change pending</option></select>
    ...
    {canReview && <AssociationMemberBulkActions role={ctx.role} visibleIds={workspace.rows.map((row) => row.id)} filteredCount={workspace.pagination.total} filterSnapshot={associationMemberFilterSnapshot(workspace.filters)} exportHref={exportHref(params)} reviewers={workspace.reviewers} />}
    <div className="overflow-x-auto rounded-xl border bg-white"><table className="w-full text-sm"><thead className="bg-slate-100 text-left text-[11px] uppercase tracking-wide text-slate-600"><tr><th className="px-3 py-2">{canReview && <input type="checkbox" aria-label="Select all visible members" data-association-member-bulk-visible-toggle />}</th><th className="px-3 py-2">Member</th><th className="px-3 py-2">Business</th><th className="px-3 py-2">Association</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Access status</th><th className="px-3 py-2">PIN expiry</th><th className="px-3 py-2">First login</th><th className="px-3 py-2">Notification</th><th className="px-3 py-2">Reviewer</th><th className="px-3 py-2"></th></tr></thead><tbody>
        {workspace.rows.map((row) => <tr key={row.id} className="border-t align-top"><td className="px-3 py-3">{canReview && <input type="checkbox" value={row.id} data-association-member-bulk-checkbox />}</td><td className="px-3 py-3"><p className="font-bold">{row.fullName ?? "Unavailable"}</p><p className="text-xs text-slate-500">{row.phone ?? "No phone"} · {row.email ?? "No email"}</p></td><td className="px-3 py-3"><p>{row.businessName ?? "Unavailable"}</p><p className="text-xs text-slate-500">{row.tradeType ?? "Trade unavailable"} · {row.lga ?? "LGA unavailable"}</p></td><td className="px-3 py-3">{row.associationName ?? "Unavailable"}</td><td className="px-3 py-3"><span className={`rounded-full border px-2 py-1 text-[11px] font-bold ${badge(row.status)}`}>{humanize(row.status)}</span></td><td className="px-3 py-3">{humanize(row.accessStatus)}</td><td className="px-3 py-3">{when(row.accessPinExpiry)}</td><td className="px-3 py-3">{row.accessFirstLoginCompletedAt ? when(row.accessFirstLoginCompletedAt) : "Password pending"}</td><td className="px-3 py-3">{humanize(row.accessNotificationStatus)}</td><td className="px-3 py-3">{row.assignedReviewerName ?? "Unassigned"}</td><td className="px-3 py-3"><Link href={`/dashboard/admin/association-members/${row.id}`} className="font-black text-emerald-800">Review</Link></td></tr>)}
        {!workspace.rows.length && queueSource.available && <tr><td colSpan={11} className="px-3 py-10 text-center text-slate-500">No members match the current queue filters.</td></tr>}
        {!workspace.rows.length && !queueSource.available && <tr><td colSpan={11} className="px-3 py-10 text-center text-rose-700">Member queue is unavailable.</td></tr>}
      </tbody></table></div>
  </section>;
}

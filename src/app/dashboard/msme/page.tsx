...
import { ProfileCompletionWelcome } from "@/components/msme/profile-completion-welcome";
import { fetchProviderQuoteInboxCount } from "@/lib/data/provider-quote-queries";
import { getProviderWorkspaceContext } from "@/lib/data/provider-operations";
import { getWorkspaceProfileCompletion, trackProfileCompletionAnalytics } from "@/lib/data/msme-profile-completion";
...
export default async function MsmePage() {
  ...
  const [
    ...
    supabase.from("provider_gallery").select("id", { count: "exact", head: true }).eq("provider_profile_id", workspace.provider.id),
    supabase
      .from("complaints")
  ...
  const completion = await getWorkspaceProfileCompletion(supabase, workspace);
  await trackProfileCompletionAnalytics(supabase, { appUserId: workspace.appUserId, msmeId: workspace.msme.id, percentage: completion.percentage, source: "dashboard" });
  const marketplaceGate = getProfileFeatureGate("marketplace", completion);
  ...
  return (
    <section className="space-y-5">
      <ProfileCompletionWelcome percentage={completion.percentage} />
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="mb-1.5 flex items-center justify-between text-sm">
                        ...
                        <p className="font-semibold text-slate-700">{completion.percentage}% Complete</p>
                      </div>
                      <div className="h-2.5 rounded-full bg-slate-100">
                        <div className="h-2.5 rounded-full bg-emerald-600" style={{ width: `${completion.percentage}%` }} />
                      </div>
                    </div>
                  </div>
              ...
              <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-3xl font-semibold text-emerald-700">{completion.percentage}% Complete</p>
                  </div>
                  <Link href="/dashboard/msme/settings" className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800">
                    Complete Profile
                  </Link>
                </div>
                ...
                {completion.missingFields.length > 0 ? (
                  <>
                    <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-slate-500">Your next profile tasks</p>
                    <ul className="mt-2 grid gap-2 sm:grid-cols-2">
                      {completion.missingFields.slice(0, 5).map((field) => (
                        <li key={field.key}>
                          <Link href={field.href} className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 hover:border-emerald-300 hover:bg-emerald-50">
                            {field.taskLabel}<ChevronRight className="h-4 w-4 text-slate-400" />
                          </Link>
                        </li>
                      ))}
                    </ul>
                ...
                <div className="mt-4 space-y-3">
                  <div className="flex items-center justify-between gap-3 text-sm">
                    <span className="flex items-center gap-2 text-slate-700">
                      {marketplaceGate.unlocked && hasPublicProfile ? <CheckCircle2 className="h-4 w-4 text-emerald-600" /> : <XCircle className="h-4 w-4 text-amber-500" />}
                      {marketplaceGate.unlocked ? "Public profile is live" : "Complete profile fields for marketplace visibility"}
                    </span>
                    {!hasPublicProfile ? (

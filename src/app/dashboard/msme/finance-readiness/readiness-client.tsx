...
const formatNaira = (value: number) => new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 }).format(value || 0);

function downloadTextPdf(lines: string[], fileName: string) {
  const safeLines = lines.map((line) => line.replace(/[()\\]/g, "")).slice(0, 45);
  const content = ["BT", "/F1 11 Tf", "50 780 Td", ...safeLines.flatMap((line, idx) => [idx === 0 ? `(${line}) Tj` : `0 -16 Td (${line}) Tj`]), "ET"].join("\n");
...
export function FinanceReadinessClient({ businessName, msmeId }: { businessName: string; msmeId: string }) {
  const [pathway, setPathway] = useState<Pathway>("loan");
  const [sectionIndex, setSectionIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, Answer>>({});
  const [showReport, setShowReport] = useState(false);
  const [fundingAmount, setFundingAmount] = useState<string>("");
  const fundingValue = Number(fundingAmount.replace(/,/g, "")) || 0;
  ...
  if (showReport) {
    return (<section className="space-y-5 pb-6"><div className="rounded-3xl border bg-white p-4 sm:p-6"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">Readiness Report Preview</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Access to Finance Readiness Index (AFRI)</h1><p className="mt-2 text-sm text-slate-600">{businessName} • {msmeId}</p><p className="text-sm text-slate-600">Funding amount needed: {formatNaira(fundingValue)}</p></div><button onClick={() => downloadTextPdf(["Access to Finance Readiness Index (AFRI)", `Business: ${businessName}`, `MSME ID: ${msmeId}`, `Pathway: ${pathway}`, `AFRI score: ${score}/100`, `Readiness band: ${score >= 80 ? "High" : score >= 60 ? "Moderate" : "Early-stage"}`, `Funding amount needed: ${formatNaira(fundingValue)}`], `finance-readiness-report-${msmeId}.pdf`)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800"><Download className="h-4 w-4" />Download PDF Report</button></div><div className="mt-4 grid gap-4 md:grid-cols-3"><div className="rounded-2xl border bg-emerald-50 p-4"><p className="text-sm">AFRI score</p><p className="text-4xl font-bold">{score}/100</p></div><div className="rounded-2xl border bg-slate-50 p-4"><p className="text-sm">Readiness band</p><p className="mt-2 text-sm font-semibold">{score >= 80 ? "High readiness" : score >= 60 ? "Moderate readiness" : "Early-stage readiness"}</p></div><div className="rounded-2xl border bg-slate-50 p-4"><p className="text-sm">Current pathway</p><p className="mt-2 text-sm font-semibold capitalize">{pathway}</p><p className="mt-1 text-xs text-slate-500">Funding signal: {formatNaira(fundingValue)}</p></div></div></div><button onClick={() => setShowReport(false)} className="rounded-xl border px-4 py-2 text-sm">Back to diagnostic</button></section>);
  }
  ...
  return (
    <section className="space-y-5 pb-24">
      <div className="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-4 sm:p-6">
        ...
        <p className="mt-2 text-sm text-slate-600">Business: {businessName} • {msmeId} • Pathway: <span className="capitalize">{pathway}</span></p>
      </div>
      <div className="rounded-2xl border bg-white p-4">
        <label htmlFor="funding-amount" className="text-sm font-semibold text-slate-900">Funding amount needed</label>
        <input id="funding-amount" type="number" min={0} placeholder="e.g. ₦1,000,000" value={fundingAmount} onChange={(e) => setFundingAmount(e.target.value)} className="mt-2 w-full rounded-lg border px-3 py-2 text-sm sm:max-w-sm" />
      </div>
      <div className="grid gap-3 md:grid-cols-3">{/* unchanged pathway cards */}
        {(Object.keys(pathwayMeta) as Pathway[]).map((k) => { const Icon = pathwayMeta[k].icon; return (<button key={k} onClick={() => setPathway(k)} className={`rounded-2xl border p-4 text-left ${pathway === k ? "border-emerald-500 bg-emerald-50" : "bg-white"}`}><div className="flex items-center gap-2"><Icon className="h-4 w-4 text-emerald-700" /><p className="font-semibold">{pathwayMeta[k].title}</p></div><p className="mt-2 text-sm text-slate-600">{pathwayMeta[k].desc}</p></button>); })}
      </div>

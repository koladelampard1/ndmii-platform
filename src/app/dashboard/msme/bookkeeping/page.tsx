...
export default function MsmeBookkeepingPage() {
  ...
  const [msmeInternalId, setMsmeInternalId] = useState<string | null>(null);
  const [formError, setFormError] = useState("");
  const [successMessage, setSuccessMessage] = useState("");
  const [loading, setLoading] = useState(false);

  const mapRowToRecord = (row: Record<string, unknown>): BookkeepingRecord => ({
    id: String(row.id ?? `bk-${Date.now()}`),
    date: String(row.record_date ?? row.date ?? ""),
    type: (String(row.record_type ?? row.type ?? "Expense") === "Income" ? "Income" : "Expense") as RecordType,
    category: String(row.category ?? ""),
    description: String(row.description ?? ""),
    amount: Number(row.amount ?? 0),
    hasReceipt: Boolean(row.receipt_url ?? row.receipt_name),
  });

  const refreshRecords = async (resolvedMsmeId: string) => {
    const { data, error } = await supabase
      .from("msme_bookkeeping_records")
      .select("id,record_date,record_type,category,description,amount,receipt_url,receipt_name")
      .eq("msme_id", resolvedMsmeId)
      .order("record_date", { ascending: false });

    if (error) {
      console.error("Insert success/error", error);
      return;
    }

    const mapped = (data ?? []).map((row) => mapRowToRecord(row as Record<string, unknown>));
    setRecords(mapped.length > 0 ? mapped : []);
  };
  ...
  useEffect(() => {
    const boot = async () => {
      ...
      const { data: byOwner } = await supabase.from("msmes").select("id").eq("created_by", user.id).maybeSingle();
      msmeId = (byOwner as { id?: string } | null)?.id ?? null;

      if (!msmeId && user.email) {
        const { data: byEmail } = await supabase.from("msmes").select("id").eq("contact_email", user.email).maybeSingle();
        msmeId = (byEmail as { id?: string } | null)?.id ?? null;
      }

      setMsmeInternalId(msmeId);
      if (msmeId) {
        await refreshRecords(msmeId);
      }
    };
  ...
  const addRecord = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.type || !form.amount || !form.category || !form.date) {
      setFormError("Please fill all required fields: record type, amount, category, and date.");
      return;
    }
    ...
    if (!msmeInternalId) {
      setFormError("Unable to resolve your MSME profile. Please refresh and try again.");
      return;
    }

    console.info("MSME id used", msmeInternalId);
    setLoading(true);

    let receiptUrl: string | null = null;
    if (form.receiptName) {
      try {
        receiptUrl = form.receiptName;
      } catch (receiptError) {
        console.warn("Receipt upload failed; saving record without receipt.", receiptError);
      }
    }

    const { error: insertError } = await supabase.from("msme_bookkeeping_records").insert({
      msme_id: msmeInternalId,
    ...
    console.info("Insert success/error", insertError ?? "success");

    if (insertError) {
      setFormError(insertError.message || "Could not save bookkeeping record.");
      setLoading(false);
      return;
    }

    setForm({ type: "Expense", amount: "", category: "", date: "", description: "", receiptName: "" });
    await refreshRecords(msmeInternalId);

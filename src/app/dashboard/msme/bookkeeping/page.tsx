...
export default function MsmeBookkeepingPage() {
  ...
  const usingCustomCategory = form.category === CUSTOM_CATEGORY_OPTION;

  const resolvedCategory = usingCustomCategory ? form.customCategory.trim() : form.category;
  ...
  const breakdown = useMemo(() => {
    const toMap = (type: RecordType) =>
      records
        ...
        .reduce<Record<string, number>>((acc, record) => ((acc[record.category] = (acc[record.category] ?? 0) + record.amount), acc), {});
  ...
  const addRecord = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!resolvedCategory) {
      setCategoryError("Select or enter a category");
      return;
    }
    ...
    setForm({ type: "Expense", amount: "", category: "", customCategory: "", date: "", description: "", receiptName: "" });
  };
  ...
  return <section className="space-y-6 pb-16"><div className="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-5 sm:p-6"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">Business Management</p><h1 className="mt-2 text-2xl font-bold text-slate-900">Bookkeeping</h1><p className="mt-2 text-sm text-slate-600">Track monthly income, expenses, and receipt evidence in a safe MVP ledger. Data is local to this session.</p></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.05fr_1fr]"><form onSubmit={addRecord} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5"><h2 className="text-lg font-semibold text-slate-900">Add record</h2><div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1 text-sm"><span className="font-medium text-slate-700">Record type</span><select value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value as RecordType, category: "", customCategory: "" }))} className="w-full rounded-lg border border-slate-300 px-3 py-2"><option>Income</option><option>Expense</option></select></label><label className="space-y-1 text-sm"><span className="font-medium text-slate-700">Amount</span><input type="number" min="0" value={form.amount} onChange={(e) => setForm((p) => ({ ...p, amount: e.target.value }))} className="w-full rounded-lg border border-slate-300 px-3 py-2" placeholder="50000" required /></label><label className="space-y-1 text-sm"><span className="font-medium text-slate-700">Category</span><select value={form.category} onChange={(e) => { setCategoryError(""); setForm((p) => ({ ...p, category: e.target.value, customCategory: e.target.value === CUSTOM_CATEGORY_OPTION ? p.customCategory : "" })); }} className="w-full rounded-lg border border-slate-300 px-3 py-2" aria-describedby="category-error" required><option value="">Select category</option>{categoryOptions.map((category) => <option key={category} value={category}>{category}</option>)}<option value={CUSTOM_CATEGORY_OPTION}>Add custom category</option></select></label><label className="space-y-1 text-sm"><span className="font-medium text-slate-700">Date</span><input type="date" value={form.date} onChange={(e) => setForm((p) => ({ ...p, date: e.target.value }))} className="w-full rounded-lg border border-slate-300 px-3 py-2" required /></label></div>
      {usingCustomCategory ? <label className="block space-y-1 text-sm"><span className="font-medium text-slate-700">Custom category</span><input value={form.customCategory} onChange={(e) => { setCategoryError(""); setForm((p) => ({ ...p, customCategory: e.target.value })); }} className="w-full rounded-lg border border-slate-300 px-3 py-2" placeholder="Enter custom category" aria-describedby="category-error" autoFocus /></label> : null}
      {categoryError ? <p id="category-error" className="text-sm text-red-600">{categoryError}</p> : null}

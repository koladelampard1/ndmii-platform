...
export default function MsmeBookkeepingPage() {
  ...
  const addRecord = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userError || !user?.user?.id) {
      setSaveError("Unable to resolve logged-in user. Please sign in again.");
      return;
    }
    ...
    if (msmeError || !msme?.id) {
      setSaveError("Unable to resolve MSME profile for this account.");
      return;
    }
    ...
    const payload = {
      ...
      receipt_url: form.receiptName || null,
      receipt_filename: form.receiptName || null,
    };

    console.log("Insert payload:", payload);

    const { error: insertError } = await supabase.from("msme_bookkeeping_records").insert(payload);

    if (insertError) {
      setSaveError(insertError.message || "Failed to save bookkeeping record.");
      return;
    }

    setRecords((previous) => [{ id: `bk-${Date.now()}`, type: form.type, amount: Number(form.amount), category: form.category, date: form.date, description: form.description, hasReceipt: Boolean(form.receiptName) }, ...previous]);

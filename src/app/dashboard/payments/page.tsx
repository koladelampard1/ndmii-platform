...
function statusLabel(status: string | null) {
  ...
  if (normalized.includes("paid") || normalized === "none" || normalized === "compliant") {
...
export default async function PaymentsPage({
  ...
  searchParams: Promise<{ receipt?: string; q?: string; status?: string; compliance?: string; taxType?: string; taxYear?: string }>;
}) {
    ...
    <section className="space-y-5 pb-2">
      ...
      <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase text-slate-500">Total Obligations</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{formatCurrency(totalObligations)}</p>
          <p className="mt-1 text-xs text-slate-500">Total tax due</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-xs text-slate-500">{totalOutstanding > 0 ? "Outstanding arrears require action" : "No outstanding arrears"}</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase text-slate-500">Compliant</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{complianceIsHealthy ? "Yes" : compliantCount}</p>
          <p className="mt-1 text-xs text-slate-500">{complianceIsHealthy ? "You are compliant" : "Compliance needs attention"}</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase text-slate-500">Overdue</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{overdueCount}</p>
          <p className="mt-1 text-xs text-slate-500">{overdueCount === 0 ? "No overdue amounts" : "Overdue obligations detected"}</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase text-slate-500">Relief / Adjusted</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{formatCurrency(reliefAmount)}</p>
          <p className="mt-1 text-xs text-slate-500">After relief adjustments</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase text-slate-500">Payment History</p>
          <p className="mt-2 text-2xl font-semibold text-slate-900">{typedHistory.length}</p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-700">{badge.body}</p>
            </div>
          </div>
          <Link
            href="/dashboard/compliance"
      ...
      <form className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          <div className="md:col-span-2">
            ...
            <input
              name="q"
              placeholder="Search by tax type, period or reference"
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase text-slate-500">Tax Type</label>
            <select name="taxType" defaultValue={params.taxType ?? ""} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase text-slate-500">Compliance Status</label>
            <select name="compliance" defaultValue={params.compliance ?? ""} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase text-slate-500">Tax Year</label>
            <select name="taxYear" defaultValue={params.taxYear ?? ""} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
          ...
          <div className="flex items-end gap-2">
            <Button className="flex-1 bg-emerald-700 hover:bg-emerald-800">
              <Filter className="mr-1.5 h-4 w-4" />
              Apply Filters
            </Button>
            <Link href="/dashboard/payments" className="inline-flex h-10 items-center px-2 text-sm text-slate-600 hover:text-slate-800">
              Reset
            </Link>
          </div>
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,2fr),minmax(320px,1fr)]">
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-slate-900">Tax Obligations</h2>
              <p className="text-sm text-slate-600">Overview of your tax/VAT obligations.</p>
            </div>
          ...
          {rows.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-y border-slate-200 bg-slate-50 text-left text-xs uppercase text-slate-500">
                ...
                <tbody>
                  {rows.map((profile) => {
                    ...
                    return (
                      <tr key={profile.msme_id} className="border-b border-slate-100">
                        <td className="px-3 py-3 font-medium text-slate-700">{profile.tax_category ?? "General"}</td>
                        <td className="px-3 py-3 text-slate-600">{profile.last_reviewed_at ? new Date(profile.last_reviewed_at).getFullYear() : "—"}</td>
                        <td className="px-3 py-3 text-slate-600">{period}</td>
                        <td className="px-3 py-3">
                          <span className={`rounded-full px-2 py-1 text-xs font-semibold ${obligationStatus.className}`}>{obligationStatus.label}</span>
                        </td>
                        <td className="px-3 py-3 text-slate-600">{Number(profile.outstanding_amount ?? 0).toLocaleString()}</td>
                        <td className="px-3 py-3 text-slate-600">{formatDate(profile.last_reviewed_at)}</td>
                        <td className="px-3 py-3">
                          {Number(profile.outstanding_amount ?? 0) > 0 ? (
                            <form action={simulatePayment} className="flex items-center gap-2">
                              <input type="hidden" name="msme_id" value={profile.msme_id} />
                              <input type="hidden" name="amount" value={Number(profile.outstanding_amount ?? 0)} />
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center">
              ...
              <h3 className="mt-3 text-lg font-semibold text-slate-900">No tax obligations</h3>
              <p className="mt-1 max-w-md text-sm text-slate-600">You have no tax obligations for the selected filters.</p>
              <Link
                href="/dashboard/payments"
                className="mt-4 inline-flex h-10 items-center rounded-md bg-emerald-700 px-4 text-sm font-medium text-white transition hover:bg-emerald-800"
              >
                View All Years
              </Link>
            </div>
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-2 flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-slate-600">Your recent tax/VAT payments.</p>
              </div>
              <Link
                href="/dashboard/payments"
            ...
            {paymentsPreview.length > 0 ? (
              <div className="space-y-2">
                {paymentsPreview.map((payment) => {
                  const paymentStatus = statusLabel(payment.status);
                  return (
                    <div key={payment.id} className="rounded-xl border border-slate-200 p-2.5">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-semibold text-slate-900">{formatCurrency(Number(payment.amount ?? 0))}</p>
                        <span className={`rounded-full px-2 py-1 text-[11px] font-semibold ${paymentStatus.className}`}>{paymentStatus.label}</span>
                      </div>
                      <p className="text-xs text-slate-600">{formatDate(payment.payment_date)} • {payment.tax_type ?? "Tax payment"}</p>
                      <p className="truncate text-xs text-slate-500">Ref: {payment.receipt_reference ?? "Not available"}</p>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-center">
                <CircleDollarSign className="mx-auto h-9 w-9 text-slate-400" />
                <h4 className="mt-2 text-sm font-semibold text-slate-900">No payments yet</h4>
                <p className="mt-1 text-xs text-slate-600">Your tax payment history will appear here once you make a payment.</p>
              </div>
            )}
          </article>
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {noticesPreview.length > 0 ? (
              <div className="mt-3 space-y-2">
                {noticesPreview.map((notice, idx) => {
                  const metadata = notice.metadata ?? {};
                  const title = String(metadata.notice_type ?? "Regulatory Notice");
                  const message = String(metadata.message ?? "Please review this notice and take action where required.");
                  return (
                    <div key={`${notice.created_at}-${idx}`} className="rounded-xl border border-slate-200 p-2.5">
                      <p className="text-xs font-semibold uppercase text-amber-700">Notice</p>
                      <p className="text-sm font-semibold text-slate-900">{title}</p>
                      <p className="line-clamp-2 text-xs text-slate-600">{message}</p>
                      <p className="mt-1 text-xs text-slate-500">{formatDate(notice.created_at)}</p>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4">
                <h4 className="text-sm font-semibold text-slate-900">No notices at the moment</h4>
                <p className="mt-1 text-xs text-slate-600">You’re all caught up! We’ll notify you if anything requires your attention.</p>
              </div>
            )}
          </article>
        </aside>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <article className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4">
          <h3 className="text-lg font-semibold text-slate-900">Stay Compliant</h3>
          <ul className="mt-2 space-y-1.5 text-sm text-slate-700">
            <li>• File your returns on time</li>
            <li>• Keep accurate records</li>
            <li>• Pay outstanding obligations</li>
            <li>• Request reliefs where eligible</li>
            <li>• Stay updated on tax regulations</li>
          </ul>
        </article>
        ...
        <article className="rounded-2xl border border-sky-200 bg-sky-50/80 p-4">
          ...
          <p className="mt-2 text-sm text-slate-700">Our support team is here to help you understand your tax obligations and compliance requirements.</p>
          <Button className="mt-4 border border-sky-200 bg-white text-sky-700 hover:bg-sky-100" type="button">
        ...
        <article className="rounded-2xl border border-amber-200 bg-amber-50/80 p-4">
          <h3 className="text-lg font-semibold text-slate-900">Tax Guides &amp; Resources</h3>
          <p className="mt-2 text-sm text-slate-700">Access helpful guides, FAQs, and resources to manage your taxes with ease.</p>
          <Link
            href="/dashboard/compliance"
            className="mt-4 inline-flex h-10 items-center rounded-md border border-amber-300 bg-white px-4 text-sm font-medium text-amber-700 transition hover:bg-amber-100"
          >
            View Tax Guide
          </Link>
        </article>
      ...
      <footer className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-2.5 text-sm text-emerald-800">
        <p>Your tax information is secure and encrypted. We follow strict security measures to protect your data.</p>
      </footer>
    </section>
  );
}

...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<{ receipt?: string }> }) {
  ...
  let profilesQuery = supabase.from("tax_profiles").select("msme_id,tax_category,vat_applicable,estimated_monthly_obligation,outstanding_amount,compliance_status").order("created_at", { ascending: false });
  ...
  return (
    <section className="space-y-6">
      <h1 className="text-2xl font-bold">{ctx.role === "msme" ? "My Tax / VAT Simulation" : "Tax / VAT Simulation"}</h1>
          ...
          return (
            <article key={profile.msme_id} className="rounded-xl border bg-white p-4">
              ...
              <div className="mt-3 space-y-1 text-sm">
                <p>Tax Category: {profile.tax_category}</p><p>VAT Applicable: {profile.vat_applicable ? "Yes" : "No"}</p><p>Estimated Monthly Obligation: ₦{Number(profile.estimated_monthly_obligation).toLocaleString()}</p><p>Outstanding Amount: ₦{Number(profile.outstanding_amount).toLocaleString()}</p><p>Compliance Status: {profile.compliance_status}</p>
              </div>

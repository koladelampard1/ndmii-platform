...
const REVIEW_STYLE_ROLES = new Set([
...
export default async function EkirsApplicationDetailPage({
  ...
  if (!detail) notFound();
  const access = await requireStateRevenueApplicationAccess({ applicationId: id, ctx, client: supabase, mode: "view" });
  const fieldOnly = access.roles.includes("field_officer") && !access.roles.some((role) => REVIEW_STYLE_ROLES.has(role));
  const evidenceRows = fieldOnly ? detail.evidence.filter((evidence) => FIELD_RELEVANT_EVIDENCE_TYPES.has(evidence.evidence_type)) : detail.evidence;
  ...
  return (
    <WorkspacePage>
      ...
      <WorkspaceContentGrid columns="lg:grid-cols-2">
        <WorkspaceSection title="Applicant and business" description="Private contact details are kept in the authorised workspace only.">
          <dl className="grid gap-3 text-sm">
            ...
            <div><dt className="font-black text-slate-500">Owner / representative</dt><dd>{fieldOnly ? "Available to EKIRS reviewers" : detail.owner_name ?? "Not provided"}</dd></div>
            {!fieldOnly ? <div><dt className="font-black text-slate-500">Email</dt><dd>{detail.contact_email}</dd></div> : null}
            <div><dt className="font-black text-slate-500">Sector</dt><dd>{detail.sector}</dd></div>
            {!fieldOnly ? <div><dt className="font-black text-slate-500">CAC / TIN</dt><dd>{detail.cac_number ?? "No CAC"} · {detail.tin ?? "No TIN"}</dd></div> : null}
          </dl>
        </WorkspaceSection>
      ...
      <WorkspaceContentGrid columns="lg:grid-cols-3">
        <WorkspaceSection title="Evidence metadata" description="Files are represented through private evidence metadata; public URLs are not exposed.">
          <div className="space-y-2">
            {evidenceRows.length ? evidenceRows.map((evidence) => (
            ...
            )) : <p className="text-sm text-slate-600">{fieldOnly ? "No field-relevant evidence is available for this assignment." : "No evidence metadata recorded."}</p>}
          </div>
        </WorkspaceSection>
        {!fieldOnly ? (
      ...
      <WorkspaceSection title={fieldOnly ? "Field outcome" : "Reviewer decision"} description={fieldOnly ? "Assigned officers can submit only their own field-verification outcome." : "Invalid transitions and self-approval are enforced server-side."}>

...
export default async function NrsRevenuePage() {
  ...
  rows.forEach((row) => trend.set(monthKey(row.created_at), (trend.get(monthKey(row.created_at)) ?? 0) + Number(row.total_amount ?? 0)));
  ...
  return (
    <section className="space-y-4">
      ...
      <article className="rounded-xl border bg-white p-4">
        ...
        <table className="mt-2 w-full text-left text-sm"><thead className="bg-slate-50"><tr><th className="px-2 py-2">Month</th><th className="px-2 py-2">Invoiced</th></tr></thead><tbody>{[...trend.entries()].sort(([a],[b]) => a.localeCompare(b)).slice(-8).map(([month, amount]) => <tr key={month} className="border-t"><td className="px-2 py-2">{month}</td><td className="px-2 py-2">{formatNaira(amount)}</td></tr>)}</tbody></table>
      </article>
      <article className="rounded-xl border bg-white p-4">

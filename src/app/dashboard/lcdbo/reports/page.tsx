...
export default async function LcdboReportsPage({ searchParams }: { searchParams: Promise<{ type?: string; state?: string; cluster?: string; partner?: string }> }) {
  ...
  if (params.state) exportParams.set("state", params.state);
  if (params.cluster) exportParams.set("cluster", params.cluster);
  if (params.partner) exportParams.set("partner", params.partner);
  ...
  return <main className="min-h-screen bg-[#F6F8FB] print:bg-white"><div className="print:hidden"><IntelligenceHeader eyebrow="Executive Reporting Centre" title="LCDBO Reports" description="Generate national, state, cluster, partner, readiness and participation views from the governed programme dataset." /></div><div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 print:max-w-none print:px-0">
    ...
    <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm print:border-0 print:p-0 print:shadow-none"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[#008751]">Executive report · Programme Estimate</p><h1 className="mt-2 text-3xl font-black text-[#0B2E59]">{REPORT_TYPES.find((item) => item.id === reportType)?.title}</h1><p className="mt-2 text-sm text-slate-500">Generated {new Date().toLocaleDateString("en-NG", { dateStyle: "long" })}. Values reflect current programme records and clearly marked demonstration data.</p></div><div className="flex flex-wrap gap-2 print:hidden"><Link href={`/api/lcdbo/reports/${reportType}${query ? `?${query}` : ""}`} className="inline-flex items-center gap-2 rounded-xl bg-[#008751] px-4 py-3 text-sm font-black text-white"><Download className="h-4 w-4" />Export CSV</Link><span className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-3 text-sm font-black text-slate-700"><Printer className="h-4 w-4" />Browser print ready</span></div></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["MSMEs", snapshot.metrics.totalMsmes], ["Clusters", snapshot.metrics.totalClusters], ["Assessments", snapshot.metrics.assessmentsCompleted], ["Active participants", snapshot.metrics.activeParticipants]].map(([label, value]) => <div key={String(label)} className="rounded-xl bg-slate-50 p-4"><p className="text-3xl font-black text-[#0B2E59]">{value}</p><p className="mt-1 text-xs font-bold text-slate-500">{label}</p></div>)}</div></article>

...
export default function AssociationAccessPage() {
  ...
  async function createPassword(event: FormEvent) {
    ...
    const data = await response.json();
    setLoading(false);
    if (!response.ok) return setError(data.error ?? "Unable to create your password.");
  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8"><section className="mx-auto max-w-lg rounded-2xl border bg-white p-6 shadow-sm sm:p-8"><Link href="/"><DbinBrandLogo textClassName="text-slate-900" /></Link>
    ...
    {done ? <><h1 className="mt-2 text-3xl font-black">Your account is ready</h1><p className="mt-3 text-slate-600">{loginEmailAvailable ? "Your password has been saved. Use your email to sign in. Phone sign-in will be enabled later." : "Your password has been saved. Phone sign-in will be enabled later. Contact the DBIN support team before trying to sign in."}</p><Link href="/login" className="mt-6 block rounded-xl bg-emerald-700 px-4 py-4 text-center text-lg font-black text-white">Go to sign in</Link></> :

...
import { AlertTriangle, BarChart3, Building2, CheckCircle2, ClipboardCheck, DatabaseZap, Factory, FileCheck2, Gauge, MapPinned, Users, UserRoundCheck } from "lucide-react";
import type { IntelligenceBreakdown, LcdboIntelligenceSnapshot } from "@/lib/data/lcdbo-intelligence";
import type { DataQualityResult, KpiSnapshot, ProgrammeHealthResult, ReportSnapshot } from "@/lib/data/lcdbo-governance";
import { LcdboCommandMetricCard, LcdboCoveragePanel, LcdboPipeline } from "@/components/lcdbo/lcdbo-visuals";

export function LcdboIntelligenceDashboard({ snapshot, quality, health, kpiSnapshots = [], reportSnapshots = [] }: { snapshot: LcdboIntelligenceSnapshot; quality?: DataQualityResult; health?: ProgrammeHealthResult; kpiSnapshots?: KpiSnapshot[]; reportSnapshots?: ReportSnapshot[] }) {
  ...
  return (
    <main className="min-h-screen bg-[#F6F8FB]">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:py-8">
        ...
        {quality && health ? <section className="grid gap-5 xl:grid-cols-[0.7fr_1.3fr]">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><DatabaseZap className="h-5 w-5 text-[#008751]" /><h2 className="font-black text-[#0B2E59]">Governance pulse</h2></div><div className="mt-5 grid grid-cols-2 gap-3"><Score label="Data quality" value={quality.score} /><Score label="Programme health" value={health.score} /></div><div className="mt-4 flex flex-wrap gap-2"><Link href="/dashboard/lcdbo/data-quality" className="rounded-xl bg-[#0B2E59] px-3 py-2 text-xs font-black text-white">Data quality</Link><Link href="/dashboard/lcdbo/geography" className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-[#0B2E59]">Geography</Link><Link href="/dashboard/lcdbo/briefings" className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-[#0B2E59]">Briefings</Link></div></article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-amber-600" /><h2 className="font-black text-[#0B2E59]">Programme monitoring alerts</h2></div><div className="mt-5 grid gap-3 sm:grid-cols-2">{health.alerts.filter((alert) => alert.count > 0).slice(0, 4).map((alert) => <div key={alert.code} className="rounded-xl bg-amber-50 p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-black text-amber-950">{alert.label}</p><span className="rounded-full bg-amber-200 px-2 py-1 text-xs font-black text-amber-900">{alert.count}</span></div><p className="mt-1 text-xs leading-5 text-amber-800">{alert.detail}</p></div>)}{!health.alerts.some((alert) => alert.count > 0) && <p className="text-sm text-slate-500">No active monitoring alerts.</p>}</div></article>
        </section> : null}

        {(kpiSnapshots.length || reportSnapshots.length) ? <section className="grid gap-5 lg:grid-cols-2"><article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><SectionTitle eyebrow="KPI trends" title="Latest governed measurements" /><div className="mt-5 grid gap-3 sm:grid-cols-2">{latestByCode(kpiSnapshots).slice(0, 6).map((item) => { const delta = kpiDelta(kpiSnapshots, item); return <div key={item.id} className="rounded-xl bg-slate-50 p-3"><div className="flex items-start justify-between gap-2"><p className="text-xl font-black text-[#0B2E59]">{Number(item.value).toLocaleString("en-NG")}</p>{delta != null ? <span className={`rounded-full px-2 py-1 text-[10px] font-black ${delta >= 0 ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"}`}>{delta >= 0 ? "+" : ""}{delta.toLocaleString("en-NG")}</span> : null}</div><p className="text-xs font-bold text-slate-500">{item.definition?.name ?? "Governed KPI"}</p><p className="mt-1 text-[10px] uppercase text-slate-400">{item.snapshot_date} · {item.frequency}</p></div>; })}</div></article><article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><SectionTitle eyebrow="Reporting governance" title="Recent report snapshots" /><div className="mt-5 space-y-2">{reportSnapshots.slice(0, 6).map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 p-3"><div><p className="text-sm font-black capitalize text-[#0B2E59]">{humanize(item.report_type)} report</p><p className="text-xs text-slate-500">{item.snapshot_date}</p></div><span className="rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black uppercase text-emerald-800">{item.frequency}</span></div>)}</div></article></section> : <article className="rounded-2xl border border-dashed border-slate-300 bg-white p-5"><h2 className="font-black text-[#0B2E59]">Governed snapshots are ready to be established</h2><p className="mt-2 text-sm leading-6 text-slate-500">Apply the Sprint C migration if needed, then capture the first KPI snapshot from the Data Quality Centre to unlock trends and reporting comparisons.</p><Link href="/dashboard/lcdbo/data-quality" className="mt-4 inline-flex rounded-xl bg-[#0B2E59] px-4 py-3 text-sm font-black text-white">Open Data Quality Centre</Link></article>}
...
function Score({ label, value }: { label: string; value: number }) { return <div className="rounded-xl bg-slate-50 p-4"><p className={`text-3xl font-black ${value >= 75 ? "text-[#008751]" : value >= 50 ? "text-amber-600" : "text-red-600"}`}>{value}</p><p className="mt-1 text-xs font-bold text-slate-500">{label} / 100</p></div>; }
function latestByCode(items: KpiSnapshot[]) { const seen = new Set<string>(); return items.filter((item) => { const code = item.definition?.code ?? item.kpi_definition_id; if (seen.has(code)) return false; seen.add(code); return true; }); }
function kpiDelta(items: KpiSnapshot[], current: KpiSnapshot) { const previous = items.find((item) => item.kpi_definition_id === current.kpi_definition_id && item.id !== current.id); return previous ? Number(current.value) - Number(previous.value) : null; }
